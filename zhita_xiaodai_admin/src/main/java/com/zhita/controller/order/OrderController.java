...
/*	//后台管理----机审订单     (公司id，page，订单号，姓名，手机号，订单开始时间，订单结束时间，风控反馈)
	...
	public void export(UserLikeParameter userLikeParameter,HttpServletRequest request, HttpServletResponse response) throws IOException {
	...
	public void exportpeople(UserLikeParameter userLikeParameter,HttpServletRequest request, HttpServletResponse response) throws IOException {
		PhoneDeal pd = new PhoneDeal();// 手机号加密解密工具类
		TuoMinUtil tm = new TuoMinUtil();// 脱敏工具类
		if ((userLikeParameter.getApplytimestart() != null && !"".equals(userLikeParameter.getApplytimestart()))
				&& (userLikeParameter.getApplytimeend() != null && !"".equals(userLikeParameter.getApplytimeend()))) {
	...
	public void exportpeopleyet(UserLikeParameter userLikeParameter,HttpServletRequest request, HttpServletResponse response) throws IOException {
		PhoneDeal pd = new PhoneDeal();// 手机号加密解密工具类
		TuoMinUtil tm = new TuoMinUtil();// 脱敏工具类
		if ((userLikeParameter.getApplytimestart() != null && !"".equals(userLikeParameter.getApplytimestart()))
				&& (userLikeParameter.getApplytimeend() != null && !"".equals(userLikeParameter.getApplytimeend()))) {
	...
	public void exportorder(OrderQueryParameter orderQueryParameter,HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		for (int i = 0; i < orderList.size(); i++) {
			BigDecimal realmoney = ordersMapper.queryrepaymoney(orderList.get(i).getId());// 该订单还款成功的还款金额--还款表
			if (realmoney == null) {
				realmoney = new BigDecimal("0.00");
			}
			BigDecimal offmoney = ordersMapper.queryrepaymoneyoff(orderList.get(i).getId());// 该订单还款成功的还款金额--线下还款表
			if (offmoney == null) {
				offmoney = new BigDecimal("0.00");
			}
			BigDecimal bankmoney = ordersMapper.queryrepaymoneybank(orderList.get(i).getId());// 该订单扣款成功的还款金额--银行卡自动扣款表
			if (bankmoney == null) {
			...
			if (orderList.get(i).getUser().getOperationTime() == null
					|| "".equals(orderList.get(i).getUser().getOperationTime())) {
				orderList.get(i).getUser().setOperationTime(null);
				;
			}
			orderList.get(i).getUser().setOperationTime(Timestamps.stampToDate(orderList.get(i).getUser().getOperationTime()));// 人审时间

			// list.get(i).setHowManyTimesBorMoney(ordersMapper.queryHow(list.get(i).getUserId()));//第几次借款

			List<DeferredAndOrder> listdefer = ordersMapper.queryDefer(orderList.get(i).getId());
			List<Offlinedelay> listdeferlay = ordersMapper.queryDeferlay(orderList.get(i).getId());
			...
			Orders os = ordersMapper.qeuryFinalDefertime(orderList.get(i).getId());// 查询最后延期时间    ---线上延期
			...
			Orders oslay = ordersMapper.qeuryFinalDefertimelay(orderList.get(i).getId());// 查询最后延期时间 ---人工延期
			if (null == oslay) {
				oslay = new Orders();
			...
			if (os.getDeferAfterReturntime().compareTo(oslay.getDeferAfterReturntime()) > 0) {
				if(os.getDeferBeforeReturntime().equals("0")&&os.getDeferAfterReturntime().equals("0")){
					orderList.get(i).setDeferBeforeReturntime("0");
					orderList.get(i).setDeferAfterReturntime("0");
				}else{
					orderList.get(i).setDeferBeforeReturntime(Timestamps.stampToDate(os.getDeferBeforeReturntime()));//延期前还款时间
					orderList.get(i).setDeferAfterReturntime(Timestamps.stampToDate(os.getDeferAfterReturntime()));// 延期后还款时间
					orderList.get(i).setPostponeDate(os.getPostponeDate());// 每次延期的天数
				}
			} else {
				if(oslay.getDeferBeforeReturntime().equals("0")&&oslay.getDeferAfterReturntime().equals("0")){
					...
					orderList.get(i).setDeferBeforeReturntime(Timestamps.stampToDate(oslay.getDeferBeforeReturntime()));//延期前还款时间
					orderList.get(i).setDeferAfterReturntime(Timestamps.stampToDate(oslay.getDeferAfterReturntime()));// 延期后还款时间
