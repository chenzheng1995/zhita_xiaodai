...
public class SourceServiceImp implements IntSourceService{
    ...
    public Map<String,Object> selectByPrimaryKey(Integer companyId,Integer id){
    	...
    	String discount=source.getDiscount();
    	source.setDiscount(discount.substring(0, discount.length() - 1));
    	source.setToken(source.getLink().substring(source.getLink().length()-6,source.getLink().length()));
